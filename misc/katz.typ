...
#let title = "intro to modern cryptography by katz and lindell"
...
#prop(
  ...
  [This proof is incomplete, since I copied down the statement wrong (originally was for all $x gt.eq 1$) \ Taking the
    natural logarithm of both sides, we obtain $ ln(1-x) lt.eq -x arrow.double \ -ln(1-x) gt.eq x $ and has derivative $ 1/(1-x ) = d/dx -ln(1-x) $ indicating
    that the derivative is negative and increasing for $x gt.eq 1$
  ],
)
...
#prop(
  [A.9: Tighter Union Bound $Pr[or_(i=1)^k E_i] lt.eq Pr[E_1] + sum_(i=2)^k Pr[E_i | overline(E_1) and dots and overline(E)_(i-1)]$],
  [By intuition],
)

#defn(
  ["Exp"ected Value of $X$],
  [
...
#prop(
  [A.10: Markov's inequality: Let $X$ be a non-negative random variable and $v gt 0$. Then $Pr[X gt.eq v] lt.eq ("Exp"[X])/v $],
  [
...
#prop(
  [Variation Bound on 0/1 random variable: For a 0/1 random variable $X_i$, it holds that $ "Var"[X_i] lt.eq 1/4 $],
  [
...
#prop(
  [: Chebyshev's Inequality: Let $X$ be a random variable and $delta > 0$. Then: $ Pr[ |X - "Exp"[X]| gt.eq delta] lt.eq "Var"[X]/delta^2 $],
  [
...
#prop(
  [: A.13: Fix $epsilon gt 0$ and $b in {0,1}$ and let $X_i$ be pairwise independent, 0/1 random variables for which \ $Pr[X_i = b] gt.eq 1/2 + epsilon$ for
  ...
  [
    ...
    Where the last line follows from the fact that $"Var"[X_i] lt.eq 1/4$ for all $i$ (proved in 1.3.8) and the corollary
    just proved (1.3.11).
...
#prop(
  [: A.14 (Chernoff Bound): Fix $epsilon gt 0$ and $b in {0,1}$ and let $X_i$ be pairwise independent, 0/1 random variables
    for which \ $Pr[X_i = b] gt.eq 1/2 + epsilon$ for all $i$. then the probability that their majority value is not $b$ is
    at most $ e^((-epsilon^2 m) \/ 2) $],
  [
...
#prop(
  [: upper and lower bounds of $"coll"(q, N)$: Claim: If $q lt sqrt(N)$ then the probability of collision $"coll"(q, N)$ is $Theta(q^2 / N)$ \
